...
crondel() {
  cronadd --rm @reboot rm -r /tmp/test &>/dev/null || true
  cronadd --rm '5 0 * * *       $HOME/bin/daily.job >> $HOME/tmp/out 2>&1' &>/dev/null || true
}
...
@test "$(description::file add multi arguments)" {
  ...
  assert_success
  assert_line --partial "Cron: @reboot rm -r /tmp/test"
...
@test "$(description::file add one argument)" {
  ...
  assert_success
  assert_line --partial 'Cron: 5 0 * * *       $HOME/bin/daily.job >> $HOME/tmp/out 2>&1'
